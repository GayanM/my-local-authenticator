 ...
 * under the License.
 */
package org.wso2.carbon.identity.application.authenticator.my.basicauth;
...
import org.wso2.carbon.identity.application.authenticator.my.basicauth.internal.BasicAuthenticatorServiceComponent;
 ...
 */
public class MyBasicAuthenticator extends AbstractApplicationAuthenticator
    ...
    private static final Log log = LogFactory.getLog(MyBasicAuthenticator.class);
    private static String RE_CAPTCHA_USER_DOMAIN = "user-domain-recaptcha";

    @Override
    public boolean canHandle(HttpServletRequest request) {
        String userName = request.getParameter(MyBasicAuthenticatorConstants.USER_NAME);
        String password = request.getParameter(MyBasicAuthenticatorConstants.PASSWORD);
        ...
        try {
            ...
            if (errorContext != null && errorContext.getErrorCode() != null) {
                ...
                if (errorCode.equals(IdentityCoreConstants.USER_ACCOUNT_NOT_CONFIRMED_ERROR_CODE)) {
                    ...
                    String redirectURL = loginPage + ("?" + queryParams) + MyBasicAuthenticatorConstants.FAILED_USERNAME
                            + URLEncoder.encode(username, MyBasicAuthenticatorConstants.UTF_8) +
                            MyBasicAuthenticatorConstants.ERROR_CODE + errorCode + MyBasicAuthenticatorConstants
                            .AUTHENTICATORS + getName() + ":" + MyBasicAuthenticatorConstants.LOCAL + retryParam;
                    ...
                    String redirectURL = loginPage + ("?" + queryParams) +
                            MyBasicAuthenticatorConstants.FAILED_USERNAME + URLEncoder.encode(request.getParameter(
                            MyBasicAuthenticatorConstants.USER_NAME), MyBasicAuthenticatorConstants.UTF_8) +
                            MyBasicAuthenticatorConstants.ERROR_CODE + errorCode +
                            MyBasicAuthenticatorConstants.AUTHENTICATORS + getName() + ":" +
                            MyBasicAuthenticatorConstants.LOCAL + retryParam;
                    response.sendRedirect(redirectURL);
                    ...
                    String redirectURL = (PASSWORD_RESET_ENDPOINT + queryParams) +
                            MyBasicAuthenticatorConstants.USER_NAME_PARAM + URLEncoder.encode(username, MyBasicAuthenticatorConstants.UTF_8) +
                            MyBasicAuthenticatorConstants.TENANT_DOMAIN_PARAM + URLEncoder.encode(tenantDoamin, MyBasicAuthenticatorConstants.UTF_8) +
                            MyBasicAuthenticatorConstants.CONFIRMATION_PARAM + URLEncoder.encode(password, MyBasicAuthenticatorConstants.UTF_8);
                    response.sendRedirect(redirectURL);
                    ...
                    if (log.isDebugEnabled()) {
                        ...
                        log.debug("username : " + request.getParameter(MyBasicAuthenticatorConstants.USER_NAME));
                        log.debug("remainingAttempts : " + remainingAttempts);
                    }

                    if (errorCode.equals(UserCoreConstants.ErrorCode.INVALID_CREDENTIAL)) {
                        retryParam = retryParam + MyBasicAuthenticatorConstants.ERROR_CODE + errorCode
                                + MyBasicAuthenticatorConstants.FAILED_USERNAME + URLEncoder
                                .encode(request.getParameter(MyBasicAuthenticatorConstants.USER_NAME),
                                        MyBasicAuthenticatorConstants.UTF_8)
                        ...
                        if (remainingAttempts == 0) {
                            if (StringUtils.isBlank(reason)) {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
                                        MyBasicAuthenticatorConstants.ERROR_CODE + errorCode + MyBasicAuthenticatorConstants.FAILED_USERNAME +
                                        URLEncoder.encode(request.getParameter(MyBasicAuthenticatorConstants.USER_NAME), MyBasicAuthenticatorConstants.UTF_8) +
                                        "&remainingAttempts=0";
                            } else {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
                                        MyBasicAuthenticatorConstants.ERROR_CODE + errorCode + "&lockedReason="
                                        + reason + MyBasicAuthenticatorConstants.FAILED_USERNAME +
                                        URLEncoder.encode(request.getParameter(MyBasicAuthenticatorConstants.USER_NAME),
                                                MyBasicAuthenticatorConstants.UTF_8) + "&remainingAttempts=0";
                            }
                        } else {
                            if (StringUtils.isBlank(reason)) {
                                redirectURL = response.encodeRedirectURL(redirectURL + ("?" + queryParams)) +
            ...
            throw new AuthenticationFailedException(e.getMessage(), User.getUserFromUserName(request.getParameter
                    (MyBasicAuthenticatorConstants.USER_NAME)), e);
        }
    }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request,
        ...
        try {
            ...
            if (userRealm != null) {
                ...
                if(isAuthenticated) {
                    isAuthorized = ((AbstractUserStoreManager) userStoreManager).isUserInRole(MultitenantUtils.getTenantAwareUsername(username),"Finance");
                }
            } else {
        ...
        if (!isAuthorized) {
            log.info("Authorization failed for user:" + username);
            throw new InvalidCredentialsException("User authorization failed due to not having Finance role",
                    User.getUserFromUserName(username));
        }
    ...
    public String getFriendlyName() {
        return MyBasicAuthenticatorConstants.AUTHENTICATOR_FRIENDLY_NAME;
    }

    @Override
    public String getName() {
        return MyBasicAuthenticatorConstants.AUTHENTICATOR_NAME;
    }
}
